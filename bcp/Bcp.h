...
enum {
  ...
  // By Allen
  CONGESTION_COUNT   = 12,	 // Number of the array size to keep track of congested packet
  CONGEST_HOP_TX     = 3,	 // Number of average transmissions per hop to define as congested
  AD_CONTROL_TRIG	 = 4	 // Number of congested packets to trigger the sink admission control
};
...
typedef nx_struct {
  ...
  //Add two fileds for Rate Control
  nx_uint8_t         valphaValue; // In the event of any  message, set valphaValue
  nx_uint8_t         valphaSeqNo; // In the event of centeral control message, set valphaSeqNo, else set 0
  nx_uint8_t         valphaUpper; // In the evnet of centeral control message, set upper bound of valpha
} bcp_data_header_t;
...
typedef nx_struct {
  ...
  //nx_uint8_t		  Va;		// By Allen, Va broadcasted from root for admission control
} bcp_beacon_header_t;
