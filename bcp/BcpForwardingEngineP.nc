...
implementation{
  ...
  /* By Allen.  The Va is the sink parameter for admission control
   * the first 4 bits are the value, the last 4 bits are the sequence number*/
  //uint8_t Va = 0xf0;
  uint8_t Valpha = 0xff;
  uint8_t VaSeq = 0;
  // start with the maximum
  uint8_t congestNodeCount = 0;
  uint16_t congestNodeId[CONGESTION_COUNT];	/* used for store the origin of 
		*	congested packets received at the sink in a beaconing period. */
  ...
  task void sendBeaconTask() {
    ...
    beaconHdr->valphaUpper = call BcpRateControlIF.getValphaUpper();
    beaconHdr->valphaSeqNo = call BcpRateControlIF.getSeqenceNo();
  ...
  task void snoopHandlerTask() {
    ...
    // Update the rate control information
    signal BcpCongestionIF.updateRateControlInfo(snoopPacket->valphaUpper,
					       snoopPacket->valphaSeqNo);
  ...
  task void sendDataTask() {
    ...
    hdr->txCount++;
    // Fill in the node valphaValue field 
    hdr->valphaValue = call BcpRateControlIF.getValphaValue();    
   
    if (hdr->valphaSeqNo < call BcpRateControlIF.getSeqenceNo()) {
      // Fill in the node valphaUpper field
      hdr->valphaUpper = call BcpRateControlIF.getValphaUpper();
      // Fill in the node valphaSeqNo field
      hdr->valphaSeqNo = call BcpRateControlIF.getSeqenceNo();
    }
  ...
  message_t* ONE forward(message_t* ONE m, uint32_t arrivalTime_p) {
    ...
    signal BcpCongestionIF.updateRateControlInfo(hdr->valphaUpper,
						  hdr->valphaSeqNo);
#ifdef LIFO
    retVal = call SendStack.pushTop(qe);
#endif
  ...
  command error_t Init.init()
  {
    ...
    notifyBurstyLinkNeighbor_m = TOS_NODE_ID;
    dbg("Forwarder_", "%s: SubPacket max payload:%hhu, BcpPacketHeader Length:%hhu.\n ", __FUNCTION__,call SubPacket.maxPayloadLength(),(uint8_t)sizeof(bcp_data_header_t));
    return SUCCESS;
  }
  ...
  event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len)
  {
    ...
    rcvPacket->hopCount++;
    dbg("Forwarder", "%s: My NODE_ID is %hhu, I'm Root:%hhu:\n", __FUNCTION__, TOS_NODE_ID, call RootControl.isRoot());
    // If I'm the root, signal receive. 
    if (call RootControl.isRoot()) {
	  // By Allen.  Update the congestion node count when a congested packet received
      
      if ( congestNodeCount < CONGESTION_COUNT && 
	   rcvPacket->txCount / rcvPacket->hopCount >= CONGEST_HOP_TX) {
	for ( i=0; i < congestNodeCount; i++ ) {
	  if (congestNodeId[i] == rcvPacket->origin)
	    break;
	}
	if ( i >= congestNodeCount && 
	     congestNodeCount < CONGESTION_COUNT ) {
	  congestNodeId[congestNodeCount] = rcvPacket->origin;
	  congestNodeCount++;
	}
      }  // End Allen
...
//                                   call AMDataPacket.destination(msg),0x51);
      ...
      // Notify congestion
      signal BcpCongestionIF.congestionNotify();
      post sendDataTask();
//      call BcpDebugIF.reportValues(sentPacket->originSeqNo,sentPacket->hopCount,sentPacket->txCount,0,0,sentPacket->origin,
//                                   call AMDataPacket.destination(msg),0x52);
      ...
      // Notify no congestion
      signal BcpCongestionIF.noCongestionNotify();
  ...
  {
    // By Allen
    //uint8_t VaTemp = 0;  // used to store Va when updating
    //uint16_t VaTemp = 0;
    uint8_t i = 0;
    ...
    // By Allen.   If it's root, update the Va value at the end of each beacon period
    if (call RootControl.isRoot()) {
      //VaTemp = Va & 0xf0;
      //VaTemp = Valpha;
      // get the first 4 bits value
      if (congestNodeCount >= AD_CONTROL_TRIG) {
	//VaTemp = (VaTemp / 2) & 0xf0;
	//Va = (Va + 1) & 0x0f;
	//Va = Va | VaTemp;
	Valpha = Valpha / 2;
	VaSeq = (VaSeq + 1) % 0xff;
      } else if ( congestNodeCount <= 1) {
	if ( Valpha < 0xff ) {
	  //VaTemp = (VaTemp + 0x10) & 0xf0;
	  //Va = (Va + 1) & 0x0f;
	  //Va = Va | VaTemp;
	  Valpha++;
	  VaSeq = (VaSeq + 1) % 0xff;
  ...
  command uint8_t      BcpPacket.getValphaValue(message_t* msg) {return getHeader(msg)->valphaValue;}
  command uint8_t      BcpPacket.getValphaSeqNo(message_t* msg) {return getHeader(msg)->valphaSeqNo;}
  ...
  event message_t* BeaconReceive.receive(message_t* msg, void* payload, uint8_t len)
  {
    ...
    signal BcpCongestionIF.updateRateControlInfo(rcvBeacon->valphaUpper,
						  rcvBeacon->valphaSeqNo);

    // By Allen.  Update the Va if sequence number is larger (newer) 
    //if ( (rcvBeacon->Va & 0x0f) > (Va & 0x0f) )
    //  Va = rcvBeacon->Va;
