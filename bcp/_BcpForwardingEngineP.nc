...
implementation{
  ...
  uint8_t Va = 0xf0;	// start with the maximum
  ...
  task void sendBeaconTask() {
	...
	// By Allen.  Store the Va value to the beacon packet
	beaconHdr->Va = Va; 
  ...
  event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len)
  {
    ...
    if (call RootControl.isRoot())
	{
				...
				rcvPacket->txCount / rcvPacket->hopCount >= CONGEST_HOP_TX)
	  {
		...
		if ( i >= congestNodeCount && congestNodeCount < CONGESTION_COUNT )
		{
...
//                                   call AMDataPacket.destination(msg),0x52);
  ...
  event void BeaconTimer.fired()
  {
	...
	if (call RootControl.isRoot()) {
		VaTemp = Va & 0xf0;		// get the first 4 bits value
